...
const PMIAnalysis = () => {
  ...
  const [[columnCode, rowCode, salesMetric], setTableCode] = useState<string[]>(['zb','sj', 'pmi_A0B01_month']);
  const [chartView, setChartView] = useState<'yoyGrowthView' | 'momGrowthView'>('momGrowthView');
  const [selectedIndicator, setSelectedIndicator] = useState<string|undefined>(salesMetric=='pmi_A0B01_month'?'A0B0101':'A090105'); // 居民消费价格指数(上月=100)
    ...
    try {
      const [currentData] = await Promise.all([
        StatsAnalysisAPI.getAnalysisData(salesMetric,{
          startTime: salesMetric=='pmi_A0B01_month'?dayjsToMonthString(dateRange[0]):dayjsToYearString(dateRange[0]),
          endTime: salesMetric=='pmi_A0B01_month'?dayjsToMonthString(dateRange[1]):dayjsToYearString(dateRange[1]),
        }),
      ])
  ...
  const calculateGrowthRate = (chartData: any[]) => {
    if (chartView == 'yoyGrowthView'&&selectedIndicator=='pmi_A0B01_month') {
      if (chartData.length < 5) return undefined;
    }
    if (chartData.length < 2) return undefined;
    
    const growthRate = chartData.map((current,index)=>{
      const previous = chartView == 'yoyGrowthView' &&selectedIndicator=='pmi_A0B01_month' && index >= 4?chartData[index - 4] : index > 0 && (chartView !== 'yoyGrowthView'||selectedIndicator!=='pmi_A0B01_month')? chartData[index - 1]:current;
    ...
    <PageContainer
      ...
      extra={[
        <RangePicker
          ...
          picker={salesMetric=='pmi_A0B01_month'?'month':'year'}
          style={{ width: 300 }}
      ...
      <Card style={{ marginBottom: 24 }}>
        <Row gutter={16}>
          <Col span={8}>
            ...
            <Segmented
              options={[
                { label: 'PPI月度', value: 'pmi_A0B01_month' },
              ]}
              value={salesMetric}
              onChange={(value) => {
                setTableCode(['zb', 'sj', value as string]);
                setSelectedIndicator(value=='pmi_A0B01_month'?'A0B0101':'A090105')
              }}
            />
          </Col>
          <Col span={8}>
          {
            salesMetric=='pmi_A0B01_month' && (
              <>
            ...
            <Col span={6}>
              <Card>
                <Statistic
                  ...
                  suffix={salesMetric=='pmi_A0B01_month'?'个月':'年'}
                />
              </Card>
            </Col>
            <Col span={6}>

...
const CPIAnalysis = () => {
  ...
  const [[columnCode, rowCode, zb], setTableCode] = useState<string[]>(['sj','zb', 'A01030G']);
  const [chartView, setChartView] = useState<'yoyGrowthView' | 'momGrowthView'>('momGrowthView');
  const [selectedIndicator, setSelectedIndicator] = useState('A01030G01'); // 居民消费价格指数(上月=100)
  ...
  const fetchData = async () => {
    ...
    try {
      const [currentData] = await Promise.all([
        StatsAnalysisAPI.getAnalysisData(`cpi_${zb}_month`,{
          startTime: dayjsToMonthString(dateRange[0]),
          endTime: dayjsToMonthString(dateRange[1]),
        }),
      ])
    ...
    >
      ...
      {!loading && currentData && (
        <>
          ...
          <Row gutter={16}>
            <Col xs={24} lg={12}>
              <ProCard title={`CPI走势分析(${chartView=='momGrowthView'?'环比':'同比'})`} headerBordered>
                <Line height={300} data={processLineChartData(growthRate||[])} xField={'time'} yField={'value'} colorField={'name'} shapeField={'smooth'}/>
              </ProCard>
            </Col>
            <Col xs={24} lg={12}>
              <ProCard title={`CPI对比分析(${chartView=='momGrowthView'?'环比':'同比'})`} headerBordered>
